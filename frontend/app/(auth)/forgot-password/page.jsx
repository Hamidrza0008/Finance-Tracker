...
import { Mail, KeyRound, ArrowLeft, Wallet, CheckCircle2, AlertCircle, X, Lock } from "lucide-react";
...
export default function ForgotPassword() {
  ...
  const resetPasswordSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/reset-password`, {
        email: form.email,
        otp: form.otp,
      ...
      console.error(error);
      showToast(error.response?.data?.message || "Invalid OTP or Error", "error");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md space-y-6 sm:space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-[10px] sm:text-xs font-bold text-slate-400 tracking-wider uppercase min-h-[16px]">
            {!pageLoading && (step === 1 ? "Enter your email to receive verification code" : `Enter the code sent to ${form.email}`)}
          </p>
        </div>

        <div className="bg-white py-6 px-4 sm:p-8 border border-[#CCE6E6]/40 rounded-2xl shadow-[0_8px_30px_-4px_rgba(13,148,136,0.03)] mx-auto w-full min-h-[220px] flex flex-col justify-center">
